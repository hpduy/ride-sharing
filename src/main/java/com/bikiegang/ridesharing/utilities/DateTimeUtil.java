...
public class DateTimeUtil {
    ...
    public long getTimeByTimeZoneFromLatLng(long epochTime, double lat, double lng){
        TimeZone timeZone = TimeZone.getTimeZone(TimezoneMapper.latLngToTimezoneString(lat,lng));
        if(timeZone != null){
            epochTime += timeZone.getOffset(epochTime);
        }
        return epochTime;
    }

    public long getTimeFromMidNight(long epochTime) {
        return epochTime % SECONDS_PER_DAY;
    }

    public int getDateFrom1970(long epochTime) {
        return (int) epochTime / SECONDS_PER_DAY;
    }

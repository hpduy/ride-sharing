   ...
   private static boolean loadBadWord(String filePath) {
     try {
       if (_hashTableBadWord == null) {
         ...
         BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputData, "UTF-8")); Throwable localThrowable2 = null;
         try { _patternBadWord = "(";
           String streamLine;
         ...
         {
           if (bufferedReader != null) if (localThrowable2 != null) try { bufferedReader.close(); } catch (Throwable x2) { localThrowable2.addSuppressed(x2); } else bufferedReader.close();  
         }
       }
     } catch (Exception e) {  }

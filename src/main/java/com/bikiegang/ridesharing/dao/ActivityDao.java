...
public class ActivityDao {
    ...
    public boolean insert(Activity obj) {
        ...
        try {
            ...
            database.getActivityHashMap().put(obj.getId(), obj);
            //Step 2: put redis
    ...
    public boolean delete(Activity obj) {
        ...
        try {
            ...
            //Step 1: put in hashmap
            database.getActivityHashMap().remove(obj.getId());
            //Step 2: put redis
            if (result) {
                //Step 3: put job gearman
                short actionType = Const.RideSharing.ActionType.DELETE;
                JobEnt job = new JobEnt(0l, 0l, obj.getClass().getName(), actionType,
                        System.currentTimeMillis(), "", JSONUtil.Serialize(obj), ActivityDA.class.getName(), "");
                result &= GClientManager.getInstance(ConfigInfo.RIDESHARING_WORKER_GEARMAN).push(job);
                if (!result) {
